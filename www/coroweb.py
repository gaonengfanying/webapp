...
def has_request_arg(fn):
    ...
    params = sig.parameters  # 含有 参数名，参数 的信息
    found = False
...
class RequestHandler(object):
    ...
    async def __call__(self, request):
        """ 分析请求，request handler,must be a coroutine that accepts a request instance as
        its only argument and returns a streamresponse derived instance """
        kw = None
            ...
            if request.method == 'GET':
                ...
                if qs:
                    ...
                    for k, v in parse.parse_qs(qs, True).items():
                        # parse a query string, data are returned as a dict. the dict keys are the unique query
                        # variable names and the values are lists of values for each name a True value indicates that
                        #  blanks should be retained as blank strings
                        kw[k] = v[0]
        if kw is None:
            # 请求无请求参数时
